    ...
    >
      <div className="container space-y-8 overflow-x-hidden">
        <div className="grid grid-cols-1 gap-12 sm:grid-cols-2 md:grid-cols-5">
          <div className="min-w-0 w-full space-y-4 break-words md:col-span-2 md:space-y-6">
            {footer.brand ? <BrandLogo brand={footer.brand} /> : null}

            {footer.brand?.description ? (
              <p
                className="text-muted-foreground text-sm max-w-md break-words"
                dangerouslySetInnerHTML={{ __html: footer.brand.description }}
              />
            ) : null}
          </div>

          <div className="col-span-1 grid min-w-0 gap-6 sm:grid-cols-3 md:col-span-3">
