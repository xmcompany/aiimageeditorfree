...
export default async function VideoPromptsPage({
  ...
  const total = await getPromptsCount({ type: 'video' });
  const data = await getPrompts({ page, limit, type: 'video' });
    ...
    columns: [
      ...
      { name: 'updatedAt', title: t('fields.updated_at'), type: 'time' },
      {
        ...
        callback: (item: Prompt) => (
          <PromptActions 
            id={item.id}
            editUrl={`/admin/video-prompts/${item.id}/edit`}
    ...
    actions: [
      {
        ...
        icon: 'RiEditLine',
        url: '/admin/video-prompts/[id]/edit',
      },
    ],
    data,
    pagination: {
