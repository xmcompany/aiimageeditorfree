...
function TaskResultPreview({ taskInfo, mediaType }: { taskInfo: string | null | undefined; mediaType: string }) {
...
function getPreviewThumbnail(task: AITask): { type: 'image' | 'video' | null; url: string | null } {
  ...
  if (info.images && info.images.length > 0) return { type: 'image', url: info.images[0].imageUrl || null };
  if (info.videos && info.videos.length > 0) return { type: 'video', url: info.videos[0].videoUrl || null };
  return { type: null, url: null };
}

export function AdminAITasksGrid({ tasks }: { tasks: AITask[] }) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      for (const task of pendingTasks) {
        try {
          const res = await fetch('/api/admin/ai-tasks/query', {
            method: 'POST',
          ...
          if (data.code === 0 && data.data) {
            setLocalTasks((prev) =>
              prev.map((t) => (t.id === task.id ? { ...t, ...data.data } : t))
            );
            // Also update selected dialog if open
            setSelected((prev) =>
              prev && prev.id === task.id ? { ...prev, ...data.data } : prev
            );
          }
        } catch {
          // ignore
    ...
    <>
      ...
      {pendingTasks.length > 0 && (
        <div className="flex items-center gap-2 rounded-md border border-yellow-200 bg-yellow-50 px-3 py-2 text-sm text-yellow-800 dark:border-yellow-800 dark:bg-yellow-950 dark:text-yellow-200">
          ...
          {pendingTasks.length} task{pendingTasks.length > 1 ? 's' : ''} in progress 鈥?auto-refreshing every 5s
        </div>
      )}
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center justify-between px-2 py-1.5">
                ...
                <span className="text-muted-foreground max-w-[80px] truncate text-xs" title={task.model ?? ''}>
        ...
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-base">
              {selected?.mediaType} 路 {selected?.model}
            </DialogTitle>
          </DialogHeader>
          ...
          {selected && (
            <div className="space-y-4">
              {/* 缁撴灉棰勮 */}
              <TaskResultPreview taskInfo={selected.taskInfo} mediaType={selected.mediaType} />
              ...
              <div className="grid gap-y-2 text-sm" style={{ gridTemplateColumns: 'auto 1fr' }}>
                ...
                <div className="text-muted-foreground pr-8">User</div>
                <div className="break-all text-xs">{(selected as any).user?.email ?? selected.userId}</div>
