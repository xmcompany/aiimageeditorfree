...
export async function POST(req: Request) {
  ...
  try {
    const { product_id, currency, payment_provider, metadata } = body;

    if (!product_id) {
      return respErr(t('messages.checkout.product_id_required'));
    }
    ...
    if (!pricingItem) {
      return respErr(t('messages.checkout.pricing_item_not_found'));
    }

    if (!pricingItem.product_id && !pricingItem.amount) {
      return respErr(t('messages.checkout.invalid_pricing_item'));
    }

    // get sign user
    const user = await getUserInfo();
    if (!user || !user.email) {
      return respErr(t('messages.checkout.no_auth'));
    }
    ...
    if (!paymentProviderName) {
      return respErr(t('messages.checkout.no_payment_provider'));
    }
    ...
    if (allowedProviders && allowedProviders.length > 0) {
      if (!allowedProviders.includes(paymentProviderName)) {
        return respErr(
          t('messages.checkout.payment_provider_not_supported', {
    ...
    if (!paymentProductId) {
      ...
      if (!checkoutPrice.amount || !checkoutPrice.currency) {
        return respErr(t('messages.checkout.invalid_checkout_price'));
      }
    } else {
