...
import { ImageIcon, RotateCw, Upload, X } from 'lucide-react';
...
const uploadImageFile = async (file: File, locale: string = 'en') => {
...
export function ImageUploader({
  ...
  onChange,
  locale = 'en',
}: ImageUploaderProps & { locale?: string }) {
  ...
  const handleFiles = (selectedFiles: File[]) => {
    ...
    Promise.all(
      newItems.map(async (item) => {
        try {
          const url = await uploadImageFile(item.file as File, locale);
          setItems((prev) => {
    ...
    >
      ...
      >
        {items.map((item) => (
          <div
            key={item.id}
            className="group border-border/40 bg-muted/30 hover:border-primary/30 relative overflow-hidden rounded-2xl border p-1 shadow-md transition-all duration-300"
          >
            <div className="relative overflow-hidden rounded-xl">
              <img
                src={item.preview}
                alt="Reference"
                className="h-32 w-32 rounded-xl object-cover transition-transform duration-500 group-hover:scale-110"
              />
              {item.size && (
                <span className="bg-black/60 backdrop-blur-md text-white absolute bottom-2 left-2 rounded-lg px-2 py-0.5 text-[10px] font-bold">
                  {formatBytes(item.size)}
                </span>
              )}
              {item.status !== 'uploading' && (
                <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/40 opacity-0 transition-opacity group-focus-within:opacity-100 group-hover:opacity-100">
                  <Button
                    type="button"
                    size="icon"
                    variant="secondary"
                    className="bg-white/20 text-white hover:bg-white/40 h-10 w-10 rounded-full shadow-xl backdrop-blur-md border border-white/20"
              ...
              {item.status === 'uploading' && (
                <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-black/60 backdrop-blur-sm text-[10px] font-bold text-white uppercase tracking-tighter">
                  <RotateCw className="h-5 w-5 animate-spin mb-1 text-primary" />
                  Uploading
                </div>
              )}
              {item.status === 'error' && (
                <div className="absolute inset-0 z-10 flex items-center justify-center bg-destructive/60 backdrop-blur-sm text-[10px] font-bold text-white uppercase">
                  Failed
                </div>
              )}
              <Button
                size="icon"
                variant="destructive"
                className="absolute top-2 right-2 z-20 h-7 w-7 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-opacity"
        ...
        {items.length < maxCount && (
          <div className="group border-primary/20 bg-primary/5 hover:border-primary/50 hover:bg-primary/10 relative overflow-hidden rounded-2xl border border-dashed p-1 shadow-sm transition-all duration-300">
            <div className="relative overflow-hidden rounded-xl">
              <button
                type="button"
                className="flex h-32 w-32 flex-col items-center justify-center gap-2 group/btn"
                onClick={openFilePicker}
              >
                <div className="bg-background/80 flex h-10 w-10 items-center justify-center rounded-full border border-primary/20 shadow-inner transition-transform group-hover/btn:scale-110">
                  <Upload className="h-5 w-5 text-primary" />
                </div>
                <span className="text-[10px] font-bold uppercase tracking-widest text-foreground/70">Upload</span>
                <span className="text-primary text-[10px] font-bold opacity-60">Max {maxSizeMB}MB</span>
              </button>
            </div>
          </div>
        )}
